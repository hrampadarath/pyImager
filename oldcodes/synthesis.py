...
def wslicfwd(guv,
    ...
    N = int(round(theta * lam))
    assert N > 1
    # Sort the u,v,w coordinates in terms of w
    p = sortw(p, None)
    nv = len(p)
    ii = range(0, nv, wstep)
...
def doimg(theta, lam, p, v, imgfn):
    ...
    :param theta:field of view in radians
    :param lam:observaing wavelength
    ...
    cdrt = imgfn(theta, lam, p, wt * v)# gridded visibilities
    drt = numpy.real(numpy.fft.ifftshift(numpy.fft.ifft2(numpy.fft.ifftshift(cdrt)))) # dirty image
    c = imgfn(theta, lam, p, wt) # gridded weights
    psf = numpy.real(numpy.fft.ifftshift(numpy.fft.ifft2(numpy.fft.ifftshift(c)))) #dirty psf
    pmax = psf.max()
    assert pmax > 0.0
