...
class WKernel(ConvFuncBase):
    """
    function to calculate the Wkernel. evaluates the function

    exp(-2j*pi*w*(1 - sqrt(1-r)))

    where w - is the w coordinate to compute the wkernel
    r - is the radial distance given by (l**2 + m**2)

    Args:
        trunc: truncation radius.
        w (float): mean w-coordinate
        r (float): radial distance from centre
    """
    #def __init__(self, trunc, r):
    #    super(WKernel, self).__init__(trunc)
    #    self.r = r

    #def f(self, radius_in_pix,w):
    #    ph = w*(1.-np.sqrt(1.-radius_in_pix))
